...
let state = { travelers: 2, days: 5, destination: null, rate: 3.65, bookingType: 'separate', meals: {}, attrs: {} };
...
// ============ CALCULATE ============
function calculate() {
    const t = Math.max(1, state.travelers || 2), d = Math.max(1, state.days || 5), r = state.rate || 3.65;
    ...
    if (budget > 0 && budgetProgress) {
        ...
        if (total <= budget) {
            if ($('budgetRemaining')) $('budgetRemaining').textContent = String.fromCodePoint(0x2705) + ' נשארו $' + fmt(budget - total);
            footer?.classList.add('in-budget');
        } else {
            if ($('budgetRemaining')) $('budgetRemaining').textContent = String.fromCodePoint(0x26A0) + ' חריגה $' + fmt(total - budget);
...
function checkAlerts() {
    ...
    if (startDate) {
        ...
        if (daysUntil > 0 && daysUntil <= 14) items.push({ icon: String.fromCodePoint(0x23F0), text: `נשארו ${daysUntil} ימים לטיסה!`, type: 'warning' });
        if (daysUntil > 30 && daysUntil <= 60) items.push({ icon: String.fromCodePoint(0x1F4A1), text: 'עוד זמן - שווה להשוות מחירים', type: 'tip' });
    }
    const budget = getVal('tripBudget');
    if (budget > 0 && currentTotal > budget) items.push({ icon: String.fromCodePoint(0x1F4B0), text: `חרגת ב-$${fmt(currentTotal - budget)}!`, type: 'error' });
...
// ============ WHATSAPP - ENHANCED ============
// Emoji constants for WhatsApp (using String.fromCodePoint for compatibility)
const WA_EMOJI = {
    plane: String.fromCodePoint(0x2708) + String.fromCodePoint(0xFE0F),
    ...
    food: String.fromCodePoint(0x1F37D) + String.fromCodePoint(0xFE0F),
    coaster: String.fromCodePoint(0x1F3A2),
    car: String.fromCodePoint(0x1F697),
    phone: String.fromCodePoint(0x1F4F1),
    shopping: String.fromCodePoint(0x1F6CD) + String.fromCodePoint(0xFE0F),
...
function buildWhatsAppMessage() {
    ...
    // Extras (Transfer, Insurance)
    const hasTransfer = isOn('checkTransfer');
    ...
    // Food
    const selectedMeals = Object.entries(state.meals).filter(([k, v]) => v);
    ...
    // Attractions
    const selectedAttrs = Object.entries(state.attrs).filter(([k, v]) => v);
